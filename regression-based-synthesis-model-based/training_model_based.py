...
def My_Custom_Generator_TS(inp_filename, out_filename, num_samples, inp_dim, out_dim, batch_size):
    ...
    """if FW_or_BW:
        x = np.memmap(inp_filename, dtype='float32', mode='r+', shape=(num_samples_FW, inp_dim), offset=0)
        y = np.memmap(out_filename, dtype='float32', mode='r+', shape=(num_samples_FW, out_dim), offset=0)
    else:"""
...
def My_Custom_Generator_GB(inp_filename, out_filename, num_samples, inp_dim, out_dim, batch_size):
    """On-the-fly data Generator for the training process (random-read)."""
    inputs = []
    targets = []
    batchcount = 0
    ...
    while True:
        for line in indices:
            ...
            linee = line
            inputs.append([abs(x[linee, dim_x:dim_x+dim_u])])
            targets.append([y[linee, :]])
            batchcount += 1
            if batchcount > batch_size:
                batch_x = np.array(inputs, dtype='float32')
...
start = time.time()
history1 = NN_TS_forw.fit(My_Custom_Generator_TS(forw_inp_TS_filename, forw_out_TS_c_filename, num_state_inp_pairs, dim_x+dim_u, dim_x, batch_size_TS),
                     steps_per_epoch=num_batches_TS, epochs=epochs_TS, verbose=1, validation_split=0, callbacks=[cp_callback_TS_forw])
history2 = NN_GB_forw.fit(My_Custom_Generator_TS(forw_inp_TS_filename, forw_out_TS_r_filename, num_state_inp_pairs, dim_x+dim_u, dim_x, batch_size_TS),
...
"""history3 = NN_TS_back.fit(My_Custom_Generator_TS(back_inp_TS_filename, back_out_TS_c_filename, num_state_inp_pairs, dim_x+dim_u, dim_x, batch_size_TS),
