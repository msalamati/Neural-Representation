...
def single_state_computation(state, controller):
    #Ctrl = np.memmap(controller_filename, dtype='float32', mode='r+', shape=(tuple(state_space_size)), offset=0)
    reach_set_ind_iter = []
    for inp_idx in range(num_dis_inputs):
        ...
        #print(time.time() - start)
        #print(len(candid_pool_ind))
        for candid in candid_pool_ind:
            if candid == (7, 6, 1):
                ddd = 1
            #for inp_idx2 in range(num_dis_inputs):
            #input2 = ind_to_cell(inp_idx2, U_range, input_space_size, eta_u, dim_u)
            ...
            #last_reach_new_tuple = [tuple(item) for item in last_reach_new_ind]
            #aaa = list(set(forward_reachable_tuple).difference(set(reachable_set_tuple)))
            #print("length is", len(aaa))
            if list(set(forward_reachable_tuple).difference(set(reachable_set_tuple))) == [] and list(set(forward_reachable_tuple)) != []:
                #  last_reach_new_ind.append(list(candid))
                #if candid == (7, 6, 1):
                    #print('passed', state, input)
                reach_set_ind_iter.append(list(candid))
                #print("YESSSS")
                #print("Here is a new reachable state:", NN_FW_inp[0:dim_x], "For the state", NN_BW_inp[0:dim_x], "using the input", input)
                controller[candid] = inp_idx
    # print(time.process_time() - start)
    return reach_set_ind_iter#, last_reach_new_ind
